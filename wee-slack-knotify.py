...
# Name of the script.
SCRIPT_NAME = 'wee-slack-knotify'

# Author of the script.
SCRIPT_AUTHOR = 'linkedlist'
...
SCRIPT_DESC = 'Sends notification through knotify'
...
def message_printed_callback(data, buffer, date, tags, is_displayed,
    ...
    parse_username(buffer, message)

    if prefix == username:
        return weechat.WEECHAT_RC_OK

    if start_time > int(date):
        return weechat.WEECHAT_RC_OK
...
def parse_username(buffer, message):
    global username
    if username is None:
        names = names_for_buffer(buffer)
        for name in names:
            if name.endswith("slack.com") and message.startswith("User name"):
